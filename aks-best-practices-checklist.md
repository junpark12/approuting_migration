...
이 문서는 [The AKS Checklist](https://www.the-aks-checklist.com/) ([GitHub: lgmorand/aks-checklist](https://github.com/lgmorand/aks-checklist))의 공개 항목(High/Medium 우선순위)에, 체크리스트 사이트에는 없는 최신 AKS 기능(Deployment Safeguards, Image Cleaner, Node Autoprovisioning, VPA, KEDA, ACR Task 자동 리빌드 등) 기반 항목을 추가하고, 내용이 겹치는 항목을 병합·정리한 것이다.
...
- **Windows 노드풀 전용 항목은 대상 클러스터가 Linux 전용이라 전부 제외**했다 (베이스 이미지/노드 OS 매핑, Windows 패치 동기화, gMSA, HostProcess, Calico Network Policy, Accelerated Networking 등 11개).
- 초안 작성 시 카테고리별로 병렬 작업하다 보니 내용이 겹치는 항목(Dockerfile lint/이미지 스캔, 로그 수집, ACR 통합, Defender for Cloud, 노드 크기 산정, requests/limits, 외부 상태 저장 등)이 있어 총 9개 항목을 병합해 정리했다.

## 현재 항목 분포 (카테고리별, 병합·Windows 제외 반영 후)
| 카테고리 | 항목 수 |
|---|---|
| Operations | 22 |
| Networking | 19 |
...
| Container | 8 |
| BC/DR | 8 |
| Additional (체크리스트 사이트에 없는 최신 기능) | 8 |
| Storage | 7 |
| Identity | 6 |
...
---
## [High] 로그 수집·저장·분석 체계 구성

**설명**: 중앙 로그 수집 체계가 없으면 노드, 시스템 파드, 애플리케이션에서 무슨 일이 일어났는지 사건 후 재구성하기가 매우 어렵습니다. 최소한 클러스터 전체 로그를 한 곳으로 모아 보존해야 할 뿐 아니라, 검색·상관분석·보존 정책까지 함께 관리해야 사후 분석과 선제 대응 모두에 활용할 수 있습니다.
...
`OPERATIONS` `ALL` `MONITORING`
---
## [High] 너무 크지도 작지도 않은 적절한 노드 크기를 선택하세요 (오토스케일러 범위 포함)

**설명**: 노드가 지나치게 크면 예약 단위가 커져 빈 리소스가 많아지고, 지나치게 작으면 시스템 파드와 애플리케이션 파드가 자주 자원 경쟁을 일으킵니다. 실제 사용률과 워크로드 특성에 맞춰 노드 크기를 조정하고, cluster autoscaler의 min/max 범위도 함께 재검토해야 비용, 성능, 가용성을 균형 있게 맞출 수 있습니다.

**확인 방법 (az cli)**:
```bash
az aks nodepool list -g $RESOURCE_GROUP --cluster-name $CLUSTER --query "[].{pool:name,vmSize:vmSize,count:count,mode:mode,minCount:minCount,maxCount:maxCount}" -o table
kubectl top nodes
...
**권장 방식으로 변경**:
```bash
az aks nodepool add -g $RESOURCE_GROUP --cluster-name $CLUSTER -n $NEW_NODEPOOL --node-vm-size $TARGET_VM_SIZE --node-count 3 --mode User --enable-cluster-autoscaler --min-count 3 --max-count 10
```

`OPERATIONS` `RESOURCE MANAGEMENT` `ALL` `COST` `FINOPS` `MONITORING` `RESOURCES`
---
...
---
## [High] 컨테이너 CPU/메모리 requests와 limits 설정 (LimitRange 포함)

**설명**: requests가 없으면 스케줄러가 정확히 배치 결정을 내리기 어렵고, limits가 없으면 한 워크로드가 노드 자원을 독점해 다른 서비스까지 불안정하게 만들 수 있습니다. 운영 환경에서는 최소/최대 자원을 명시해 QoS를 분명히 하고 HPA/VPA가 의미 있는 지표를 보도록 해야 하며, 팀별 기본값은 네임스페이스 LimitRange로 강제해 설정 누락 자체를 막는 것이 좋습니다.
...
`APPLICATION` `RESOURCE MANAGEMENT` `ALL` `RESOURCES` `MULTI-TENANCY` `RESILIENCY`
---
## [High] Dockerfile 정적 분석(Lint) 및 이미지 컴플라이언스 검사

**설명**: 취약한 베이스 이미지, `latest` 태그, root 사용자 실행, 불필요한 패키지 설치, 과도한 capability, 금지 포트 노출, 시크릿 파일 포함 여부는 단순 CVE 스캔만으로는 놓치기 쉽고, 문제의 상당수는 이미 Dockerfile 단계에서 시작됩니다. Dockerfile 자체를 빌드 전에 정적 점검(hadolint)하고, 빌드된 이미지의 구성 모범사례까지 검사(dockle)하는 두 단계를 빌드 파이프라인에 함께 넣어야 운영에 올라가기 전에 보안 부채와 컴플라이언스 위반을 조기에 차단할 수 있습니다.
...
`APPLICATION` `ALL` `SECURITY`
---
## [High] 컨테이너 이미지 취약점(CVE) 스캔 및 임계값 정책 적용

**설명**: 실제로 배포되는 아티팩트는 소스 코드가 아니라 이미지이므로, 이미지가 레지스트리에 푸시된 뒤에 취약점을 찾는 것보다 빌드 단계에서 바로 실패시키는 편이 수정 비용과 배포 위험이 훨씬 낮습니다. High/Critical, 수정 가능 여부(ignore-unfixed), 예외 승인 기준으로 자동 차단하는 임계값 정책을 빌드 파이프라인에 두고, 여기에 더해 ACR/Defender for Containers의 레지스트리 취약점 평가를 함께 켜 두면 새 CVE가 공개될 때 오래 저장된 이미지까지 재평가되어 드리프트를 잡을 수 있습니다.
...
```
```bash
# 레지스트리 차원의 지속적 취약점 재평가(Defender for Containers)
...
`CONTAINER` `APPLICATION` `ALL` `SECURITY`
---
...
`CLUSTER SECURITY` `ALL` `SECURITY` `OPERATIONS`
---
## [High] Microsoft Defender for Containers/Cloud 활성화 및 지속 모니터링

**설명**: 운영 AKS는 이미지 취약점만이 아니라 런타임 이상 징후와 Kubernetes 위협 탐지, 그리고 클러스터 보안 상태(secure score)까지 하나의 체계로 봐야 합니다. Defender for Containers를 활성화하고 Log Analytics로 신호를 모으면 배포 전후 위험을 상관 분석할 수 있고, assessment/secure score를 주기적으로 점검하면 미적용 정책이나 과도한 권한 같은 구조적 문제를 경고 개수보다 먼저 파악할 수 있습니다.
...
`CLUSTER SECURITY` `ALL` `SECURITY` `OPERATIONS` `COMPLIANCE`
---
...
---
## [High] 이미지 레지스트리는 ACR 같은 프라이빗 레지스트리 사용 (비밀번호 없는 통합)

**설명**: 퍼블릭 레지스트리 의존도가 높으면 공급망 통제, 네트워크 안정성, 이미지 승인 절차가 모두 느슨해집니다. 운영 클러스터는 프라이빗 레지스트리로 공급 경로를 고정해야 하며, 이때 이미지 pull 자격 증명을 시크릿으로 배포하는 대신 AKS-ACR 통합으로 클러스터 정체성에 직접 pull 권한을 부여해 시크릿 누출·만료 대응 부담을 없애야 합니다.
...
`CLUSTER SECURITY` `IDENTITY` `ALL` `COMPLIANCE` `SECURITY`
---
...
az aks nodepool add -g $DR_RG --cluster-name $DR_CLUSTER -n userdr --node-vm-size Standard_D4ds_v5 --node-count 3
```
...
**설명**: Pod나 노드 수명주기와 함께 사라지는 상태를 클러스터 내부에 두면 스케일아웃과 복구가 어려워집니다. 데이터베이스뿐 아니라 세션, 캐시, 업로드 파일처럼 애매한 상태까지 포함해서 봐야 하며, StatefulSet이 꼭 필요한지 먼저 검토하고 가능하면 관리형 PaaS로 분리해 데이터 복제와 백업 책임을 플랫폼으로 넘기는 편이 안정적입니다. 이런 부가 상태를 방치하면 운영 중 점점 늘어나 롤링 업그레이드와 노드 교체 시 복구 순서를 복잡하게 만듭니다.
...
**설명**: 노드당 Pod 수가 많아질수록 이미지 캐시, 컨테이너 로그, 임시 파일이 OS 디스크를 빠르게 소모합니다. Ephemeral을 쓸 수 없는 환경에서는 OS 디스크 크기와 VM 계열을 보수적으로 잡아야 디스크 포화로 인한 kubelet 불안정성을 줄일 수 있습니다. 특히 로깅이 많은 워크로드에서 체감 차이가 큽니다.
